...
def Mostrar_senha_func():
    if en_senha.cget('show') == '*':
        en_senha.config(show='')
    else:
        en_senha.config(show='*')


def Mostrar_senha_usua():
    if en_cliente_senha.cget('show') == '*':
        en_cliente_senha.config(show='')
    else:
        en_cliente_senha.config(show='*')
...
check_senha_func = Checkbutton(frame_func, bg=azul_esc, image=on_image, selectimage=off_image, indicatoron=False, bd=0,
                               command=lambda: Mostrar_senha_func())
# posicionamento na tela
lb_func.pack()
...
check_senha_us = Checkbutton(frame_cliente, bg=branco, image=on_image, selectimage=off_image, indicatoron=False, bd=0,
                             command=lambda: Mostrar_senha_usua())
# posionamento na tela
lb_cliente.pack()
